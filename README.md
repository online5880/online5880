# 안녕하세요, Unreal 실무 개발자 **mane(박만혜)** 입니다.

저는 Unreal Engine 기반 프로젝트에서
**시스템 구조 설계, 플러그인 개발, 시뮬레이션 기능 구현**을 중심으로 일하고 있습니다.

새로운 기술을 꾸준히 학습하고,
배운 내용을 실무에 적용해 **재사용 가능한 형태로 정리**하는 것을 중요하게 생각합니다. 🌱

---

## 🛠️ Core Skills

- Unreal Engine 5
- C++
- Blueprint
- Plugin Architecture
- Simulation Systems / UI Flow / Level Streaming / Optimization

---

## 📫 How to Reach Me & 📄 Blogs

<table align="center">
<tr>
<td align="center">
<a href="mailto:gjtjqkr5880@gmail.com">
<img src="https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white"/><br>
Gmail
</a>
</td>
<td align="center">
<a href="https://www.youtube.com/@mane588">
<img src="https://img.shields.io/badge/YouTube-FF0000?style=flat&logo=youtube&logoColor=white"/><br>
YouTube
</a>
</td>
<td align="center">
<a href="https://online-unreal.tistory.com/">
<img src="https://img.shields.io/badge/Tistory-000000?style=flat&logo=tistory&logoColor=white"/><br>
Unreal Engine Blog
</a>
</td>
</tr>
</table>

---

<div align="center">
<a href="https://github.com/online5880/github-readme-activity-graph" style="display:inline-block;">
<img
src="https://github-readme-activity-graph.vercel.app/graph?username=online5880&theme=react-dark&bg_color=20232a&hide_border=true&line=58A6FF&color=58A6FF"
width="50%"
/>
</a>

<a href="https://github.com/devxb/gitanimals" style="display:inline-block;">
<img
src="https://render.gitanimals.org/farms/online5880"
width="35%"
/>
</a>
</div>
